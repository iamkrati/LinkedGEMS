...
if (isset ($_GET['user_name']))
{
   $get_username = $_GET['user_name'];
    $query="SELECT * FROM Signup WHERE Email='$get_username' limit 1";
    $res=mysqli_query($con,$query);
    $row1=mysqli_fetch_assoc($res);
    $clicked_name=$row1['Username'];
    $clicked_email=$row1['Email']; 
    $clicked_company=$row1['jobposition']; 
    $clicked_tech=$row1['techstack']; 
    $clicked_role=$row1['jobrole']; 
    
//  to indicate the number of message 
   
    $read="UPDATE users_chat SET Msg_status=read WHERE Sender_email='$clicked_email'";
...
<!-- drop-down-->
     ...
     <div style="background-color:#37517e;" class="row heading">
